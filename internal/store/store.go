...
func (s *Store) UpsertCommitment(ctx context.Context, id, provider, consumer, root []byte, chunks, expires, block uint64, metadata string) error {
	_, err := s.pool.Exec(ctx, `
        INSERT INTO commitments (
            commitment_id, provider, consumer, merkle_root,
            total_chunks, expires_at, block, status, raw_event, metadata
        ) VALUES ($1,$2,$3,$4,$5,$6,$7,'registered', jsonb_build_object('block', $7::bigint), $8)
        ON CONFLICT (commitment_id) DO UPDATE SET
            ...
            updated_at = now()
    `, id, provider, consumer, root, int64(chunks), int64(expires), int64(block), metadata)
	return err
}
...
func (s *Store) ListCommitments(ctx context.Context) ([]Commitment, error) {
	rows, err := s.pool.Query(ctx, `
        SELECT commitment_id, provider, consumer, merkle_root,
               ...
               COALESCE(status, 'registered'), COALESCE(deliver_url, ''), COALESCE(metadata, '')
        FROM commitments
        ORDER BY updated_at DESC
    `)
	if err != nil {
...
func scanCommitment(row scanner) (Commitment, error) {
	...
	var status, deliver, meta string
	if err := row.Scan(&id, &provider, &consumer, &root, &chunks, &expires, &block, &status, &deliver, &meta); err != nil {
		if err == pgx.ErrNoRows {
