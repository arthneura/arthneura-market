...
func mountOffers(mux *http.ServeMux, db *store.Store) {
	...
	mux.HandleFunc("GET /v1/offers/{id}/stamp", func(w http.ResponseWriter, r *http.Request) {
		...
		schemaID := off.Schema
		if schemaID == "" {
			schemaID = listing.Schema
		}
		ready := false
		reason := "commitment not linked"
		if err := schema.Same(listing.Schema, schemaID); err != nil {
			reason = err.Error()
		} else if off.CommitmentID != "" {
		...
		writeJSON(w, http.StatusOK, map[string]any{
			...
			"ready":             ready,
			"reason":            reason,
			"register_commitment": map[string]any{
				"pallet":            "VectorDb",
				"call":              "register_commitment",
				"metadata":          schemaID,
				"merkle_root":       off.MerkleRoot,
				"total_chunks":      off.TotalChunks,
				"expires_in_blocks": off.ExpiresInBlocks,
